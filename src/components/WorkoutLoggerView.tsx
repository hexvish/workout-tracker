...
import { Play, Check, Plus, Trash2, Clock, Trophy, Dumbbell, Timer } from 'lucide-react';
import confetti from 'canvas-confetti';
import { AppState, Exercise, WorkoutExercise, WorkoutSession, WorkoutSet, BodyPartCategory, BODY_PART_COLORS } from '../types';
...
export const WorkoutLoggerView: React.FC<WorkoutLoggerViewProps> = ({
  ...
  const [selectedBodyPartFilter, setSelectedBodyPartFilter] = useState<BodyPartCategory | 'all'>('all');
      ...
      sets: [
        {
          id: `set-${Date.now()}-1`,
          setNumber: 1,
          weightKg: exercise.metricType === 'weight_reps' ? 50 : undefined,
  ...
  const filteredModalExercises = allExercises.filter((ex) => {
    if (selectedBodyPartFilter === 'all') return true;
    return ex.muscleGroup === selectedBodyPartFilter;
  });
    ...
    <div className="space-y-4 pb-28 animate-fadeIn">
      ...
      {sessionExercises.length === 0 && (
        <div className="p-8 rounded-3xl bg-[#16161A] border border-dashed border-white/10 text-center space-y-3">
          ...
          <div>
            ...
            <p className="text-xs text-zinc-400 mt-1">Tap below to pick exercises by body part category.</p>
          </div>
        </div>
      )}
      ...
      <div className="space-y-4">
        {sessionExercises.map((ex, exIndex) => {
          const bodyPartKey = ex.muscleGroup as BodyPartCategory;
          const colorInfo = BODY_PART_COLORS[bodyPartKey];
            ...
            <div key={exIndex} className="bg-[#16161A] border border-white/5 rounded-2xl p-4 shadow-sm">
              <div className="flex items-center justify-between mb-3">
                ...
                <div className="flex items-center gap-1">
                  {/* Optional Manual Rest Timer Trigger */}
                  <button
      ...
      <div className="space-y-2">
        ...
        >
          <Plus className="w-4 h-4 text-indigo-400" /> Choose Exercise by Body Part
        </button>
      ...
      {/* Body Part Categorized Exercise Picker Modal */}
      {showAddModal && (
        <div className="fixed inset-0 z-50 bg-[#0A0A0C]/80 backdrop-blur-sm flex items-end sm:items-center justify-center p-2 sm:p-4 animate-fadeIn">
          <div className="bg-[#16161A] border border-white/5 rounded-3xl w-full max-w-md max-h-[85vh] flex flex-col overflow-hidden shadow-2xl">
            <div className="p-4 border-b border-white/5 flex items-center justify-between">
              <h3 className="text-sm font-bold text-white uppercase tracking-wider">Exercise Library</h3>
              <button
            ...
            {/* Body Part Category Filters */}
            <div className="p-3 border-b border-white/5 overflow-x-auto flex gap-1.5 scrollbar-none">
              <button
              ...
              {(Object.keys(BODY_PART_COLORS) as BodyPartCategory[]).map((cat) => {
                const colorInfo = BODY_PART_COLORS[cat];
                const isSelected = selectedBodyPartFilter === cat;
                return (
                  <button
                    key={cat}
                    onClick={() => setSelectedBodyPartFilter(cat)}
                    ...
                    <div className="flex items-center gap-2.5">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-zinc-400">
                          {ex.category} • <span className="uppercase text-zinc-300 font-semibold">{ex.muscleGroup}</span>
                          {ex.isCustom ? ' • Custom' : ''}
                        </p>
                      </div>
                    </div>
